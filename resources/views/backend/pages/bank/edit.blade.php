...
@section('content')
    ...
    @include('backend.layouts.error_msg')
        <div class="block block-rounded">
            ...
            <form class="js-validation" id='form' action='{{ url('bank/' . $bank_info->id . '/update')}}' method="POST" onsubmit="return validate_name(event)">
                @csrf
                <div class="block block-rounded">
                        ...
                        <div class="row items-push ml-10">
                            <div class="col-lg-8 col-xl-5">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="name" name="name" value="{{ $bank_info->name }}" placeholder="Enter a name.." >
                                    <span id="error_name" class="m-2" style="color:red;  font-size: 14px;"></span>

                                </div>
                                <div class="form-group">
                                    <label for="val-suggestions">Address</label>
                                    <textarea class="form-control" id="address" name="address" rows="5" placeholder="What it is used for?">{{ $bank_info->address ?? "" }}</textarea>
                                </div>
                            </div>
                        </div>
                        <!-- END Regular -->
    ...
    <script>
        function validate_name(e) {
            $.ajax({
                type: 'POST',
                async:false,
                url: '{{ url('bank/'. $bank_info->id .'/validate_name') }}',
                data: $('#form').serialize(),
